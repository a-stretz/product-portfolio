        ...
        {pathResult?.blockers && pathResult.blockers.length > 0 && (
          <div className="report-section">
            ...
            {pathResult.blockers.map((b, i) => (
              <div key={i} style={{ marginBottom: 10, padding: '12px 14px', background: 'var(--red-light)', border: '1px solid #fca5a5', borderRadius: 'var(--radius)' }}>
                <div style={{ fontWeight: 600, fontSize: 13, color: 'var(--red)', marginBottom: 4 }}>⚠ {b.message}</div>
                <div style={{ fontSize: 12, color: '#991b1b' }}>{b.detail}</div>
              </div>
            ))}
          </div>
        )}
