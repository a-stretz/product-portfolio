        ...
        <div className="card" style={{ marginBottom: 16 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 6, marginTop: 8 }}>
            {INTERVENTION_PATHS.map(p => {
              ...
              return (
                <div key={p.id} style={{ padding: '10px 8px', borderRadius: 'var(--radius)', border: `2px solid ${isRec ? 'var(--accent)' : isCapped ? '#fca5a5' : 'var(--border)'}`, background: isRec ? 'var(--accent-light)' : isCapped ? 'var(--red-light)' : 'var(--surface-2)', textAlign: 'center', fontSize: 11 }}>
          ...
          {pathResult.pathCapped && (
            <div style={{ marginTop: 10, padding: '10px 12px', background: '#fef3c7', border: '1px solid #fcd34d', borderRadius: 'var(--radius)', fontSize: 12, color: '#92400e' }}>
          ...
          {pathResult.blockers.length > 0 && (
            <div style={{ marginTop: 10 }}>
              ...
              {pathResult.blockers.map((b, i) => <div key={i} style={{ fontSize: 12, color: '#991b1b', marginBottom: 4 }}>⛔ {b.message}</div>)}
            </div>
          )}
        </div>
      )}
